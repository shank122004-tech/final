...
(function (global) {
  ...
  window.BA = {
    ...
    _openBattle(battleId, battleData) {
      ...
      // Start polling this battle with fast interval
      if (this._pollGameInterval) clearInterval(this._pollGameInterval);
    ...
    _renderBattleRoom(battle) {
      ...
      if (battle.status === 'waiting') {
        ...
        body.innerHTML = `
          ...
          <div class="ba-section-title">👥 Players Joined</div>
          <div class="ba-players-list" style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.02);">
            ${displayNames.map(n => `<div class="ba-player-chip" style="display:inline-block;padding:8px 12px;margin:4px;background:rgba(108,99,255,0.15);border-radius:8px;color:var(--text-primary);font-size:12px;font-weight:600;border:1px solid rgba(108,99,255,0.2);">${n}</div>`).join('')}
          </div>
          ${(() => {
            if (isCreator) {
