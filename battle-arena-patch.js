...
(function (global) {
  ...
  window.BA = {
    ...
    _renderBattleList(battles, searchQuery) {
      ...
      html += `
        <div style="display:flex;gap:8px;margin-bottom:12px;">
          ...
          />
          ${searchQuery ? `<button onclick="BA._clearSearch()" style="padding:0 12px;background:rgba(255,255,255,0.06);border:1px solid rgba(108,99,255,0.25);border-radius:10px;color:rgba(26,26,38,0.6);cursor:pointer;flex-shrink:0;">✕</button>` : ''}
        </div>`;
      ...
      (async () => {
        if (typeof window.checkBattleAccess === 'function') {
          ...
          if (!access.unlimited && access.limit && access.used !== undefined) {
            ...
            const limitHtml = `
              <div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:10px;padding:10px 12px;margin-bottom:12px;font-size:13px;color:rgba(26,26,38,0.8);">
                <div style="font-weight:700;color:var(--text-primary);margin-bottom:4px;">⚔️ Free Daily Battles: ${access.used}/3 Used</div>
                <div style="font-size:12px;color:rgba(26,26,38,0.6);">${remaining} free battles remaining today · Upgrade to Premium for unlimited</div>
              </div>`;
      ...
      if (isCreator) {
        ...
        html += `
          <div class="ba-gate-box">
            ...
            <div style="font-size:12px;color:rgba(26,26,38,0.6);margin-bottom:14px;line-height:1.5;">
            ...
            <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">
              <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.3);border-radius:10px;padding:10px 12px;">
                <div><div style="font-size:13px;font-weight:700;color:var(--text-primary);">⚔️ Basic</div><div style="font-size:11px;color:rgba(26,26,38,0.70);">5 battles/month</div></div>
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(239,68,68,0.12);border:1.5px solid rgba(239,68,68,0.5);border-radius:10px;padding:10px 12px;position:relative;">
                ...
                <div><div style="font-size:13px;font-weight:700;color:var(--text-primary);">⚔️⚔️ Pro</div><div style="font-size:11px;color:rgba(26,26,38,0.70);">19 battles/month</div></div>
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(245,158,11,0.07);border:1px solid rgba(245,158,11,0.35);border-radius:10px;padding:10px 12px;">
                <div><div style="font-size:13px;font-weight:700;color:var(--text-primary);">⚔️🏆 Academy</div><div style="font-size:11px;color:rgba(26,26,38,0.70);">29 battles/month</div></div>
      ...
      if (!allBattles || allBattles.length === 0) {
        ...
        if (demoToShow.length > 0 && realBattles.length === 0) {
          html += `<div style="background:rgba(108,99,255,0.08);border:1px solid rgba(108,99,255,0.15);border-radius:10px;padding:8px 12px;margin-bottom:12px;font-size:12px;color:rgba(26,26,38,0.70);display:flex;align-items:center;gap:8px;">
        ...
        allBattles.forEach(b => {
          if (b._isDemo && !demoSepAdded && realBattles.length > 0) {
            demoSepAdded = true;
            html += `<div style="text-align:center;padding:8px 0 4px;font-size:11px;color:rgba(26,26,38,0.65);letter-spacing:0.05em;">── More Open Battles ──</div>`;
          }
      ...
      const _renderLobby = () => {
        ...
        body.innerHTML = `
          ...
          <div style="background:rgba(74,222,128,0.08);border:1px solid rgba(74,222,128,0.2);border-radius:12px;padding:14px;margin:12px 0;text-align:center;">
            ...
            <div style="font-size:12px;color:rgba(26,26,38,0.70);margin-top:4px;" id="demo-lobby-count">${pc}/${MAX_PLAYERS} joined · ${MAX_PLAYERS - pc} slots left</div>
          </div>
          <div class="ba-section-title">👥 Players Joined</div>
          <div class="ba-players-list" id="demo-players-list">
            ${demoState.players.map(n => `<div class="ba-player-chip">${n === myName ? '<span style="color:#5b46d4;">'+n+' (You)</span>' : n}</div>`).join('')}
          </div>
          <div id="demo-waiting-msg" style="text-align:center;padding:14px;color:rgba(26,26,38,0.70);font-size:13px;">⏳ Battle starts when all ${MAX_PLAYERS} slots are filled…</div>
          <div id="demo-ai-loading" style="display:none;text-align:center;padding:12px;font-size:12px;color:rgba(26,26,38,0.70);">
    ...
    _renderBattleRoom(battle) {
      ...
      // FIX: Get names for ALL players (not just ready ones)
      ...
      // Get display names for ready players
      const displayNames = readyPlayersList.map(uid => playerNames[uid] || 'Player');
      ...
      if (battle.status === 'waiting') {
        ...
        body.innerHTML = `
          ...
          <div style="background:rgba(74,222,128,0.08);border:1px solid rgba(74,222,128,0.2);border-radius:12px;padding:14px;margin:12px 0;text-align:center;">
            ...
            <div style="font-size:12px;color:rgba(26,26,38,0.70);margin-top:4px;">${playerCount}/${MAX_PLAYERS} ready · ${MAX_PLAYERS - playerCount} waiting</div>
          </div>
          <div class="ba-section-title">👥 Players Joined</div>
          <div class="ba-players-list">
            ${displayNames.map(n => `<div class="ba-player-chip">${n}</div>`).join('')}
          </div>
          ${(() => {
            if (isCreator) {
    ...
    _renderActiveQuiz(battle) {
      ...
      // FIX: Include user progress in hash to ensure per-user rendering
